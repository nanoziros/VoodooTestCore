		...
		private void GetPos(ref float _Pos)
		{
			switch (m_PosType)
			{
				case PositionType.SCREEN:
					_Pos = UnityEngine.Input.mousePosition.x;
					break;

				case PositionType.SCREEN_PERCENT:
					_Pos = (UnityEngine.Input.mousePosition.x / Screen.width);
					break;

				case PositionType.WORLD:
					m_PosBuffer.Set (UnityEngine.Input.mousePosition.x, 0.0f, GetDepth ());
